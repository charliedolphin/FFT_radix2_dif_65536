// difradix2_65536
// 2-65536 point fft radix 2 (Decimation in Frequency) Algorithm
// 25.12.2015 by Dmitry Chistyakov 
//
// Updates:
//      25.12.2015 - release.
//      01.04.2017 - optimized binary-inversion function.
//      13.04.2017 - reduced number of math operations.
//*****************************************************************************
//*****************************************************************************
#include <stdio.h>
#include <stdlib.h>
...
float r_snk, j_snk, r_sns2k, j_sns2k, number;  
...
int main()
{
    ...
    while (step>=2) {
      for (k=0; k<step/2; k++) {
            for (n=0; n<N/step; n++) {
                ...
                r_snk=fft_r[index0]+fft_r[index1];
                j_snk=fft_j[index0]+fft_j[index1];
                r_sns2k=m_real(exp_r[i], exp_j[i], 
                    fft_r[index0]-fft_r[index1], 
                    fft_j[index0]-fft_j[index1]);
                j_sns2k=m_im(exp_r[i], exp_j[i], 
                    fft_r[index0]-fft_r[index1], 
                    fft_j[index0]-fft_j[index1]);
    ...
    // FFT end.
    // Calculate unsorted amplitude spectrum
    for (i=0; i<N; i++) {
